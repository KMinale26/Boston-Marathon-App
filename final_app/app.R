    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel(
          ...
          p("As Data scientists we can’t simply waste away the findings of our projects. Data is used to help answer real-world problems and this project is no exception to that notion. For our work, we play the role of two different groups of people interested in the marathon. The sports analyst (think of organizations such as ESPN or NCAA) and the marathon organizers. Sports will often come to the Boston Marathon to gauge the next generation of runners or possibly even recruit them. Joan Benoit (Maine resident) won the Boston Marathon in 1979 and 1983. She then competed in the 84’ Olympics and won a gold medal. Another more recent example, Peres Jepchirchir who was originally from Kenya participated in and won the 2022 Boston Marathon. She then went to the Olympics and won a gold medal in the women’s marathon. We see how important this event is in gauging potential recruits, so as a sports analyst, there are a few important variables. Performance-focused variables such as overall placement, finish time in min, sec, and hrs, and even half-time for pacing observation are all important to remember. But one of the most surprising aspects of this project was how bib number was one of the chief variables for us to look at. An immense amount of data can be extracted when taking this variable and comparing it with the others. The reason the bib number is so important is because of how the BAA (Boston Athletic Association) prioritizes bib number."), 
          p("When you want to sign up for the Boston Marathon, you need to first register for the qualifying runs. When you participate in these qualifiers you must receive a qualifying time to be eligible for the race:"),
          tags$div(
...
-The p-value is <2.2e-16 which tells us that the bib number and finish time have a statistically significant relationship here. 
...
-F-statistic is 7524 with the earlier p-value being < 2e-16 means this model is significant"),
          p("The coefficient being positive for bib numbers does support the idea that higher bib numbers lead to slower finish times however, there seems to be a bit more variability than we would like since we want a strong conclusion here. We need a stronger model."),
          p("Creating a regression model comparing bib numbers and overall placement:"),
          p("Here we see the bib_number coefficient is 0.6904 which tells us that for every single increase in the bib number, the place you will finish in the race increases by 0.6904. Again looking at the p-value tells us that this relationship is significant with the R^2 value accounting for over 60% of the variance in overall placement. Here we see a much stronger model that we can base a conclusion that does make sense when looking at the variables. While finish time does tell us the actual performance of the runner with their time, there is a significant amount of variability that could go into it. (Weather conditions, morning routine, or even fatigue). Overall placement however is not affected by this so it builds a stronger model."),
...
server <- function(input, output, session) {
  ...
  filtered <- reactive({
    req(input$update) # Wait for button press
    filtered[filtered$bib_number >= input$bibRange[1] & 
                    filtered$bib_number <= input$bibRange[2], ]
  })
  output$scatterPlot <- renderPlot({
    plot <- ggplot(filtered(), aes(x = bib_number, y = place_overall)) +
      geom_point(alpha = 0.5) +
